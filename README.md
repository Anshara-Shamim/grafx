...
| `gunicorn` | Production server for deployment on Railway |
| `psycopg2-binary` | PostgreSQL database driver required for Railway deployment |
...
**Error:**
SQLAlchemy raises a `_handle_dbapi_exception` error when trying to insert a user into the database.

**Cause:**
The `DATABASE_URL` environment variable was not linked to the app service on Railway. The app could not connect to the PostgreSQL database.

**Solution:**
1. Go to your **Postgres** service on Railway → **Variables** tab
...
**Note:** If your database URL starts with `postgres://`, update it in your code:
```python
...
**Error:**
Worker fails to boot with `ModuleNotFoundError: No module named 'psycopg2'`.

**Cause:**
The PostgreSQL driver `psycopg2` was missing from `requirements.txt`. Without it, SQLAlchemy cannot connect to a PostgreSQL database.
